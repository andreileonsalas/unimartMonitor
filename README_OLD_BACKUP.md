...
2. Busca el Pull Request activo
...
**Modo Manual (URLs específicas):**
```bash
node scraper.js --mode=manual --url="https://www.unimart.com/products/..."
```
- Scrapea URLs específicas sin esperar al weekly
- **Uso:** Productos nuevos que no están en sitemap aún
...
- `shopify_gid`: Shopify Global ID for the variant (optional)
- `stock`: Stock quantity available (optional, null if unavailable)
...
The scraper uses **parallel processing** with segmented workers:

- **Parallel workers**: Default is 8 (configurable via `--segments` parameter)
- **Parallel requests**: `PARALLEL_REQUESTS = 30` - Products scraped in parallel per batch
- **Sitemap parallel**: `SITEMAP_PARALLEL_REQUESTS = 15` - Sitemaps fetched in parallel

#### How Parallel Scraping Works

1. **Segmentation**: Products divided into N segments (default 8)
2. **Independent workers**: Each worker processes its segment independently
3. **Database merging**: After all workers complete, databases are merged
4. **Deduplication**: Products appearing in multiple sitemaps are deduplicated by URL

**Performance**:
- Full scrape: ~45-60 minutes with 8 workers
- Daily updates: ~15 minutes with 8 workers
- Scalable: Adjust workers via `--segments=N` parameter
...
- **Dónde comenzar**: revisar `scraper.js` — funciones importantes: `initDatabase()`, `fetchProductSitemaps()`, `scrapeAndSave(db, url)`, `main()`.
...
- **Qué necesita la IA para entender/editar correctamente**:
   ...
   - Entender el esquema de la base de datos (tablas `products`, `variants`, `prices`).
...
- **Acciones seguras que puede automatizar la IA**:
   - Actualizar selectores de `scrapeSimpleProduct()` si la estructura HTML cambia.
   - Ajustar `PARALLEL_REQUESTS` y delays si se observan bloqueos o lentitud.
...
- **Prueba recomendada después de cambiar el scraper**:
   1. Ejecutar `npm test` para validar que no se rompió nada.
   2. Ejecutar manual scraping con una URL de prueba.
   3. Revisar que la base de datos se actualice correctamente.
...
2. **Check database exists**: Ensure `prices.db.gz` is committed and pushed to `main`
3. **Wait for deployment**: GitHub Pages can take 2-5 minutes to deploy after merging
4. **Hard refresh**: Press Ctrl+F5 (Windows/Linux) or Cmd+Shift+R (Mac) to clear cache
5. **Check browser console**: Open DevTools (F12) and look for any JavaScript errors
6. **Verify SQL.js loads**: The page needs to load SQL.js from CDN - check Network tab in DevTools

**Common issues**:
- **Database not on main**: Ensure `prices.db.gz` is on the `main` branch
...
- Verify `prices.db.gz` exists in the repository on the `main` branch  
- Check browser console for errors (F12 → Console tab)

### Scraper not finding products
- Check if unimart.com's sitemap structure has changed
- Use manual scraping for specific products: `node scraper.js --mode=manual --url="..."`
- Check the products table: `sqlite3 prices.db "SELECT COUNT(*) FROM products;"`
